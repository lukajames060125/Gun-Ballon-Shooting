...
const App: React.FC = () => {
  ...
  const handleAction = (e: React.MouseEvent) => {
    // Only handle left clicks for popping/missing
    if (e.button !== 0) return;
    
    if (gameState.ammo <= 0 || gameState.status !== 'playing' || showPrizeOverlay || showHowToPlay) return;

    const target = e.target as HTMLElement;
    const balloonClickable = target.closest('.balloon-clickable');
    
    if (balloonClickable) {
      const id = balloonClickable.getAttribute('id');
      if (id) {
    ...
    // It was a miss if we didn't hit a UI element
    if (target.closest('.inventory-item') || target.closest('.prize-slot') || target.closest('.mute-toggle') || target.closest('.nav-button')) return;
  ...
  const getGunTransform = () => {
    ...
    const snipeScale = 0.7; // Standard scale for non-sniping
    return `translateX(${moveX}px) translateY(${recoilY}px) rotate(${recoilRot}deg) scale(${snipeScale})`;
  };
        ...
        <div className="bg-wood-dark border-8 border-amber-900 max-w-2xl w-full p-10 rounded-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 text-left font-bungee">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-amber-900/20 p-4 border border-amber-900 rounded-xl">
                <p className="text-yellow-500 text-sm mb-1 uppercase tracking-wider font-black">Controls</p>
                <p className="text-white text-xs">Left Click: <span className="text-red-500">Shoot</span><br/>Right Click (Hold): <span className="text-cyan-500">SNIPE / SCOPE</span></p>
              </div>
            </div>
          </div>
  ...
  const SniperOverlay = () => {
    if (!isSniping || gameState.status !== 'playing') return null;
    return (
      <div className="scope-vignette">
        <div className="scope-lines">
          <div className="scope-circle">
             ...
             <div className="scope-center-dot"></div>
             {/* Integrated ammo counter for scope mode */}
             <div className="absolute top-[20%] right-[15%] flex flex-col items-center bg-black/40 p-2 rounded border border-red-600/30">
               <div className="text-[10px] font-bungee text-red-500 mb-1 opacity-70">AMMO</div>
               <div className="text-2xl font-bungee text-white">{gameState.ammo.toString().padStart(2, '0')}</div>
             </div>
             <div className="absolute bottom-16 text-red-600 font-bungee text-[10px] animate-pulse tracking-widest bg-black/40 px-3 py-1 rounded">PRECISION ENGAGED</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div 
      className={`relative w-screen h-screen overflow-hidden market-bg flex flex-col items-center justify-center crosshair ${isShaking ? 'shake' : ''}`} 
    ...
    >
      ...
      {gameState.status === 'playing' && (
        <div className={`relative w-full h-full flex flex-col items-center justify-center transition-all duration-300 ${isSniping ? 'snipe-zoom' : ''}`}>
          ...
          <div className="relative w-full max-w-lg h-[35vh] flex items-center justify-center p-6 bg-wood border-[10px] border-amber-950 rounded shadow-2xl z-20 -translate-y-24 scale-[0.85]">
            <div className="w-full h-full bg-black/40 rounded shadow-inner p-4">
              <div className="grid grid-cols-10 grid-rows-6 w-full h-full gap-2">
                {balloons.map((b) => (
                  <div key={b.id} id={b.id} className="balloon-clickable relative flex items-center justify-center">
      ...
      {gameState.status === 'playing' && (
        <div 
          className={`fixed bottom-[-140px] left-1/2 -translate-x-1/2 pointer-events-none transition-all duration-150 ease-out z-[105] ${isSniping ? 'opacity-0 translate-y-20' : 'opacity-100 translate-y-0'}`} 
          style={{ transform: getGunTransform() }}
        >
          {renderCurrentWeapon()}
          <div className="absolute top-[10%] left-1/2 -translate-x-1/2 bg-black/90 px-4 py-2 rounded-lg border-2 border-yellow-500/50 z-50">
            <div className="text-xl text-yellow-500 font-bungee tracking-[0.2em]">{gameState.ammo.toString().padStart(2, '0')}</div>
          </div>
        </div>
      )}
